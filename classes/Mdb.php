...
class Mdb
{
	...
	public function getDb($attempt = 0)
	{
		...
		try {
			if ($this->mongoClient == null) {
				$this->mongoClient = new MongoDB\Client("mongodb://$mongoServer:$mongoPort");
			}
			if ($this->db == null) {
				$this->db = $this->mongoClient->zkillboard;
			}
	...
	{
		return new \MongoDB\BSON\UTCDateTime(time() + $delta);
	}
	...
	{
		$doc = $this->findDoc($collection, $query);
		return $doc !== null;
	}
	...
	public function insert($collection, $values)
	{
		...
		try {
			...
			return $this->getCollection($collection)->insertOne($values);
		} finally {
			sem_release($sem);
		}
	}
	...
	public function save($collection, $document)
	{
		...
		try {
			...
			if (isset($document['_id'])) $this->set($collection, $document, $document);
			else {
	...
	public function set($collection, $key, $value, $multi = false)
	{
		...
		try {
			...
			if ($multi) return $this->getCollection($collection)->updateMany($key, ['$set' => $value]);
			return $this->getCollection($collection)->updateOne($key, ['$set' => $value]);
		} finally {
			sem_release($sem);
		}
	}
	...
	public function find($collection, $query = [], $sort = [], $limit = null, $includes = [])
	{
		...
		if ($limit != null) $include['limit'] = $limit;
		if (sizeof($sort)) $include['sort'] = $sort;
	...
	public static function group(string $collection, array $keys = [], array $query = [], array $count = [], array $sum = [], array $sort = [], int $limit = 0)
	{
		global $debug;
