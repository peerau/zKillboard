...
class MongoFilter
{
	...
	$options = ['$project' > ['_id' => 0, 'killID' => 1], 'sort' => [$sortKey => $sortDirection]];
	if (!isset($parameters['nolimit'])) $options['limit'] = $limit;
	if ($page > 0) $options['skip'] = ($page * $limit);
	    ...
	    foreach ($parameters as $key => $value) {
		    ...
		    switch ($key) {
			    ...
			    case 'date':
				    ...
				    $and[] = ['dttm' => ['$gte' => new \MongoDB\BSON\UTCDateTime($time)]];
				    $and[] = ['dttm' => ['$lt' => new \MongoDB\BSON\UTCDateTime($time + 86400)]];
				    break;
			    case 'relatedTime':
				    $time = strtotime($value);
				    $exHours = isset($parameters['exHours']) ? (int) $parameters['exHours'] : 1;
				    $and[] = ['dttm' => ['$gte' => new \MongoDB\BSON\UTCDateTime($time - (3600 * $exHours))]];
				    $and[] = ['dttm' => ['$lte' => new \MongoDB\BSON\UTCDateTime($time + (7200 * $exHours))]];
				    break;
			    case 'pastSeconds':
				    ...
				    $and[] = ['dttm' => ['$gte' => new \MongoDB\BSON\UTCDateTime(time() - $value)]];
				    break;
			    case 'beforeKillID':
			    ...
			    case 'endTime':
				    $time = strtotime($value);
				    $and[] = ['dttm' => ['$lte' => new \MongoDB\BSON\UTCDateTime($time)]];
				    break;
			    case 'orderBy':
    ...
    public static function getFirstKillIDTime($time)
    {
	    ...
	    $time = $time - ($time % 60);
	    $iter = $mdb->getCollection("killmails")->find(['dttm' => ['$gte' => new \MongoDB\BSON\UTCDateTime($time)]])->sort(['killID' => 1])->limit(1);
	    $row = $iter->next();
